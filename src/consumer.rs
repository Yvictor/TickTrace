...
mod tests {
    ...
    async fn test_consumer_batch_writing() -> Result<()> {
        ...
        let mut consumer = QuoteConsumer::new(receiver, 2, output_dir.clone(), 100_000)?;
    ...
    async fn test_consumer_partial_batch() -> Result<()> {
        ...
        let output_dir = temp_dir.path().to_str().unwrap().to_string();
        let (sender, receiver) = flume::bounded(100);
        let mut consumer = QuoteConsumer::new(receiver, 3, output_dir.clone(), 100_000)?;
    ...
    async fn test_parquet_file_schema() -> Result<()> {
        ...
        let output_dir = temp_dir.path().to_str().unwrap().to_string();
        let (sender, receiver) = flume::bounded(100);
        let mut consumer = QuoteConsumer::new(receiver, 1, output_dir.clone(), 100_000)?;
